...
{
    const u32 HSEMHZ = HSE_VALUE/1000000;
    ...
    if (errorStatus == SUCCESS)
    {
        ...
        switch(speedm)
        {
          case RCC_Speed_100mHz:
            ...
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ  , 100*2, 2, 15);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_168mHz:
            FLASH_SetLatency(FLASH_Latency_5);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ  , 168*2, 2, 15);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_192mHz:
            FLASH_SetLatency(FLASH_Latency_5);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ  , 192*2, 2, 15);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_200mHz:
            FLASH_SetLatency(FLASH_Latency_5);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ  , 200*2, 2, 15);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_210mHz:
            FLASH_SetLatency(FLASH_Latency_6);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ  , 210*2, 2, 15);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_220mHz:
            FLASH_SetLatency(FLASH_Latency_6);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ/4, 220/2, 2,  7);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_230mHz:
            FLASH_SetLatency(FLASH_Latency_7);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ/4, 230/2, 2,  7);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_234mHz:
            FLASH_SetLatency(FLASH_Latency_7);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ/4, 234/2, 2,  7);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_240mHz:
            FLASH_SetLatency(FLASH_Latency_7);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ/4, 240/2, 2,  7);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_250mHz:
            FLASH_SetLatency(FLASH_Latency_7);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ/4, 250/2, 2,  7);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_260mHz:
            FLASH_SetLatency(FLASH_Latency_7);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ/4, 260/2, 2,  7);      //Divisor by 4 because TSL use not stable Quartz
            break;
          case RCC_Speed_280mHz:
            FLASH_SetLatency(FLASH_Latency_7);
            RCC_PLLConfig(RCC_PLLSource_HSE, HSEMHZ/4, 280/2, 2,  7);      //Divisor by 4 because TSL use not stable Quartz
            break;
          default:
...
////////////////////
      ...
      SPI_InitStructure.SPI_BaudRatePrescaler   = SPI_BaudRatePrescaler_256; // fPCLK/128
...
//    GPIO_Init(GPIOA,&is); 
      ...
      if (spd)
      {
        ...
        if (spd==5)
        {
          GPIO_PinAFConfig(GPIOA, GPIO_PinSource8, GPIO_AF_TIM1);    //CLK MATRIX
        } else {
          GPIO_PinAFConfig(GPIOA, GPIO_PinSource8, GPIO_AF_MCO);    //CLK MATRIX
        }
      } else {
        GpioInit_CLK_EPM_OFF();
      }
    ...
    {
      case 1:
        RCC_MCO1Config(RCC_MCO1Source_PLLCLK, RCC_MCO1Div_3);    //  RCC_MCO_SYSCLK);         // 78 mHz
        break;
      case 2:
        RCC_MCO1Config(RCC_MCO1Source_PLLCLK, RCC_MCO1Div_4);    //  RCC_MCO_PLLCLK_Div2);    // 39 mHz
        break;
      case 3:
        RCC_MCO1Config(RCC_MCO1Source_HSI,    RCC_MCO1Div_1);    //  RCC_MCO_HSE);            // 12 mHz
        break;
      case 4:
        RCC_MCO1Config(RCC_MCO1Source_HSE,    RCC_MCO1Div_1);    //  RCC_MCO_HSI);            // 8 mHz
        break;
      case 5:
        //chipAY_TimerSetupX(TIM1, 100, 10, 1);
        break;
      default:
        RCC_MCO1Config(RCC_MCO1Source_HSI,    RCC_MCO1Div_1);    //  RCC_MCO_NoClock);
        break;
    }
}
...
void Init(void)
{
      ...
      //RCC_Configuration(RCC_Speed_168mHz);
